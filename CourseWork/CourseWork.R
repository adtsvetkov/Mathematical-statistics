...
example_foo <- function(AfrAA, NorAA, k)
{
  ...
  #рисуем облако точек в координатах m1 и m2
  
  #для Африки
  
  png(width = 800, height = 500, filename ="Values cloud for Africa.png")
  
  plot(Africa_par[, 2], Africa_par[, 3], col = "red", pch = 19, cex = 2, xlab = "m1", ylab = "m2", main = "Values cloud for Africa")
  
  dev.off()
  
  #для севера
  
  png(width = 800, height = 500, filename ="Values cloud for North.png")
  
  plot(North_par[, 2], North_par[, 3], col = "blue", pch = 19, cex = 2, xlab = "m1", ylab = "m2", main = "Values cloud for North")
  
  dev.off()
  
  #общее
  
  png(width = 800, height = 500, filename ="Values cloud.png")
  
  plot(North_par[, 2], North_par[, 3], col = "blue", pch = 19, cex = 2, xlab = "m1", ylab = "m2", main = "Values cloud", xlim=range(my_m1))
  points(Africa_par[, 2], Africa_par[, 3], pch = 24, cex=2, col="red", bg="green")
  
  legend("topright", c("Africa","North"), col = c("red", "blue"), pch = c(24, 19))
  dev.off()
